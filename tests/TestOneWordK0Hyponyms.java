...
public class TestOneWordK0Hyponyms {
    ...
    // Tests that a word not in the dataset returns an empty list
    @Test
    public void jellyTest() {
        NgordnetQueryHandler studentHandler = AutograderBuddy.getHyponymsHandler(
                WORD_HISTORY_SIZE3_FILE, YEAR_HISTORY_FILE, SYNSET_SIZE16_FILE, HYPONYM_SIZE16_FILE);
        List<String> words = new ArrayList<>();
        words.add("jelly");
    ...
    // Tests that a word belonging to multiple synsets returns all hyponyms
    ...
    public void testChangeK0() {
        ...
        NgordnetQuery nq = new NgordnetQuery(words, 0, 0, 0);
        String actual = studentHandler.handle(nq);
        ex = "[alteration, change, demotion, increase, jump, leap, modification, saltation, transition, variation]";
